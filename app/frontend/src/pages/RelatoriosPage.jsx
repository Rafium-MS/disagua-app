...
import { exportToXLSX, exportToPDF } from '../utils/exporters.js'

function makeExportRows(rowsByBrand){
  const rows = []
  for (const [marca, rowsArr] of Object.entries(rowsByBrand)){
    for (const r of rowsArr){
      ...
      rows.push({
        Marca: marca, Loja: r.loja, Data: new Date(r.data).toLocaleDateString('pt-BR'),
        '20L': currencyBRL(r.valor_20l), '10L': currencyBRL(r.valor_10l),
        '1500ml': currencyBRL(r.valor_1500ml), 'CX Copo': currencyBRL(r.valor_cx_copo),
        Vasilhame: currencyBRL(r.valor_vasilhame), Total: currencyBRL(total),
      })
    }
  }
  return rows
...
export default function RelatoriosPage(){
  ...
  function ensureSample(){
    ...
    if (brands.length===0 || stores.length===0){ alert('Cadastre marcas e lojas para gerar dados de exemplo.'); return }
    const gen = []
    for (let i=0;i<80;i++){
  ...
  const grouped = useMemo(()=>{
    const g = {}; for (const row of filtered){ (g[row.marca] ??= []).push(row) } return g
  }, [filtered])

  const totals = useMemo(()=>{
    let t20=0,t10=0,t15=0,tc=0,tv=0,gt=0
    for (const r of filtered){ t20+=+r.valor_20l||0; t10+=+r.valor_10l||0; t15+=+r.valor_1500ml||0; tc+=+r.valor_cx_copo||0; tv+=+r.valor_vasilhame||0; gt+= (+r.valor_20l||0)+(+r.valor_10l||0)+(+r.valor_1500ml||0)+(+r.valor_cx_copo||0)+(+r.valor_vasilhame||0) }
  ...
  return (<div className="grid gap-4">
    <Card title="Gerar Relatório" actions={<Button variant="subtle" onClick={ensureSample}>Gerar dados de exemplo</Button>}>
      <div className="grid md:grid-cols-4 gap-3">
        ...
        <Select label="Marca" value={marcaId} onChange={e=>setMarcaId(e.target.value)}><option value="">Todas</option>{brands.map(b=> <option key={b.id} value={b.id}>{b.marca}</option>)}</Select>
      </div>
    </Card>
    <Card title="Resumo" actions={<><button onClick={()=>exportToXLSX(makeExportRows(grouped))} className="bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded-lg text-sm mr-2">Exportar Excel</button><button onClick={()=>exportToPDF(makeExportRows(grouped))} className="bg-slate-800 hover:bg-slate-900 text-white px-3 py-2 rounded-lg text-sm">Exportar PDF</button></>}>
            ...
            <div key={marca} className="border rounded-xl overflow-hidden">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                  <thead><tr className="bg-white text-left">{["Loja","Data","20L","10L","1500ml","CX Copo","Vasilhame","Total"].map(h=> <th key={h} className="px-3 py-2">{h}</th>)}</tr></thead>
                  <tbody>{rows.map((r, idx) => {
                    const total = (+r.valor_20l||0)+(+r.valor_10l||0)+(+r.valor_1500ml||0)+(+r.valor_cx_copo||0)+(+r.valor_vasilhame||0)
                    return (<tr key={idx} className="border-t">
