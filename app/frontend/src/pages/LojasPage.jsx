...
export default function LojasPage(){
  ...
  const [storeForm, setStoreForm] = useState({
    marca_id:'', loja:'', cod_disagua:'', local_entrega:'', endereco:'',
    municipio:'', uf:'', valor_20l:0, valor_10l:0, valor_1500ml:0, valor_cx_copo:0, valor_vasilhame:0
  })
  ...
  function addBrand(e){
    ...
    setBrands(arr => [...arr, { id, ...brandForm }])
  ...
  function addStore(e){
    e.preventDefault()
    const id = crypto.randomUUID()
    setStores(arr => [...arr, { id, ...storeForm, marca_id: String(storeForm.marca_id) }])
    setStoreForm({ marca_id:'', loja:'', cod_disagua:'', local_entrega:'', endereco:'', municipio:'', uf:'', valor_20l:0, valor_10l:0, valor_1500ml:0, valor_cx_copo:0, valor_vasilhame:0 })
  }

  function delBrand(id){
    if(!confirm('Excluir marca e lojas associadas?')) return
    setBrands(arr => arr.filter(b => b.id !== id))
    setStores(arr => arr.filter(s => s.marca_id !== id && s.marca_id !== String(id)))
  }
      ...
      <Card title="Cadastro de Lojas">
        <form onSubmit={addStore} className="grid gap-3">
          ...
          <div className="grid md:grid-cols-5 gap-3">
            ...
            <Select label="UF" value={storeForm.uf} onChange={e=>setStoreForm({...storeForm, uf:e.target.value})} required>
              <option value="">Selecione</option>
                ...
                <div key={b.id} className="border rounded-xl overflow-hidden">
                  <div className="flex items-center justify-between bg-slate-100 px-4 py-2">
                    ...
                    <div className="flex gap-2">
                      <Button variant="danger" onClick={()=>delBrand(b.id)}>Excluir Marca</Button>
                    </div>
                  </div>
                      ...
                      <table className="min-w-full text-sm">
                        <thead>
                          <tr className="bg-white">
                            {["Loja","Local","Endereço","Município","UF","20L","10L","1500ml","CX Copo","Vasilhame","Ações"].map(h => <th key={h} className="px-3 py-2 text-left">{h}</th>)}
                          </tr>
                        </thead>
                        <tbody>
                          {brandStores.map(s => (
                            <tr key={s.id} className="border-t">
                              ...
                              <td className="px-3 py-2">{s.local_entrega}</td>
