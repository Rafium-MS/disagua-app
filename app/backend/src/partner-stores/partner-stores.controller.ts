...
export class PartnerStoresController {
  ...
  @Post(':partnerId/:storeId') connect(@Req() req:any, @Param('partnerId') partnerId:string, @Param('storeId') storeId:string){
    return this.service.connect(req.user.orgId, partnerId, storeId);
  }
  @Delete(':id') disconnect(@Req() req:any, @Param('id') id:string){ return this.service.disconnect(req.user.orgId, id); }
}
