...
export default function ParceirosPage(){
  ...
  const [form, setForm] = useState({
    cidade: '', estado: '', parceiro: '', distribuidora: '',
    cnpj_cpf: '', telefone: '', email: '',
    cx_copo: 0, dez_litros: 0, vinte_litros: 0, mil_quinhentos_ml: 0, vasilhame: 0,
  })
  ...
  function onSubmit(e){
    ...
    setPartners(arr => [...arr, { ...form, id }])
    setForm({ cidade:'', estado:'', parceiro:'', distribuidora:'', cnpj_cpf:'', telefone:'', email:'', cx_copo:0, dez_litros:0, vinte_litros:0, mil_quinhentos_ml:0, vasilhame:0 })
  }
            ...
            <table className="min-w-full text-sm">
              <thead>
                <tr className="bg-slate-100 text-left">
                  {["Cidade","UF","Parceiro","Distribuidora","CNPJ/CPF","Telefone","Email","Ações"].map(h => <th key={h} className="px-3 py-2">{h}</th>)}
                </tr>
              </thead>
              <tbody>
                {partners.map(p => (
                  <tr key={p.id} className="border-b">
                    ...
                    <td className="px-3 py-2">{p.cnpj_cpf}</td>
