...
export default function Conectar() {
  ...
  const connections = connectionsRepo.all()
  ...
  const lojasFiltradas = useMemo(
    () =>
      lojas.filter(
        (l: any) => (!uf || l.uf === uf) && (!municipio || l.municipio.toLowerCase().includes(municipio.toLowerCase()))
      ),
    [lojas, uf, municipio]
  )

  const lojaAlreadyLinked = lojaId ? connectionsRepo.byLoja(lojaId) : null
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="grid grid-cols-3 gap-2">
          {lojasFiltradas.map((l: any) => {
            const linked = connectionsRepo.byLoja(l.id)
            const partnerName = linked ? parceiros.find((p) => p.id === linked.parceiroId)?.parceiro : null
            return (
              <button
                key={l.id}
                className={`text-left border rounded p-2 ${lojaId === l.id ? 'ring-2 ring-blue-500' : ''} ${
                  linked ? 'opacity-60' : ''
                }`}
      ...
      <section className="space-y-2">
        <h2 className="font-medium">Vínculos atuais</h2>
